...
// Add a PDF carousel to the page
function createPDFCarousel(pdfs, targetId, options = {}) {
    const section = document.getElementById(targetId);
    if (!section) {
        console.error(`Element with id '${targetId}' not found.`);
        return;
    ...
    const { controls = true, interval = 5000 } = options;
    ...
    pdfs.forEach((pdf, index) => {
        ...
        embedElement.src = pdf.src;
        embedElement.type = 'application/pdf';
        embedElement.style.width = '100%';
        embedElement.style.height = '800px';
        embedElement.style.border = 'none';

        innerCol.appendChild(embedElement);
        innerRow.appendChild(innerCol);
        carouselItem.appendChild(innerRow);
        carouselInner.appendChild(carouselItem);
    });
...
// Add an image carousel to the page
function createImageCarousel(images, targetId, options = {}) {
    const section = document.getElementById(targetId);
    if (!section) {
        console.error(`Element with id '${targetId}' not found.`);
        return;
    ...
    const { interval = 5000, height = '600px' } = options;
    ...
    images.forEach((image, index) => {
        ...
        innerCol.appendChild(imgElement);
        innerRow.appendChild(innerCol);
        carouselItem.appendChild(innerRow);
        carouselInner.appendChild(carouselItem);
    });
